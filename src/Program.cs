...
namespace OutlookCO2
{
    class Program
    {
        ...
        const string dataSource1 = "https://charting.numberlens.com/api/teamearth/getdailyco2?authtoken=D43026302F294A5784F7512A8969FE37";
        ...
        private static async Task<Dictionary<string, string>> GetData()
        {
            ...
            Console.WriteLine($"Getting data from {(dataSource1.Contains("?") ? dataSource1.Substring(0, dataSource1.IndexOf("?")) : dataSource1)}");
            // regex = @"(?s)id=""currentReadingValue"">([\d\.]*)\sppm.*?id=""lastYearReadingValue"">([\d\.]*)\sppm";
            regex = @"(?s)""CurrentIndexValue"":([\d\.]+).*?""PerviousIndexValue"":([\d\.]+)";
